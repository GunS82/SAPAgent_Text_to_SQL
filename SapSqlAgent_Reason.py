...
from ws_session import NlmkChatSession
...
def run_sgr_agent_adaptive(
    ...
    api_key: str = os.getenv("NLMK_API_KEY"),
    assistant_id: str = os.getenv("ASSISTANT_ID"),
):
    if not api_key or not assistant_id:
        raise ValueError("Требуются переменные окружения API_KEY и ASSISTANT_ID.")

    # Очистка консоли и вывод запроса в начале
    clear_console()
    print_query(nl_query)

    chat = NlmkChatSession(base_url=base_url, ws_url=ws_url, api_key=api_key, assistant_id=assistant_id)
    chat.connect()
    db = DBLogger()
    db.connect()
    ...
    try:
        for _ in range(1, max_steps + 1):
            payload = build_incremental_payload(messages, last_sent_idx)
            if not payload:
                raise RuntimeError("Нет новых сообщений для отправки ассистенту.")

            resp_text = chat.send_and_wait(payload, timeout_sec=180)
            last_sent_idx = len(messages)
            messages.append({"role":"assistant","content":resp_text})
            db.log_message(turn_index=len(messages)-1, role="assistant", content=resp_text, meta={"raw_stream": True}, dialog_id=final_dialog_id)
            ...
            if not job:
                ...
                db.log_message(turn_index=len(messages)-1, role="user", content=correction, meta={"reason":"bad_json"}, dialog_id=final_dialog_id)
                if bad_json_streak >= 3:
                    example = {"next_step":{"kind":"select_tables","thought":"Пояснение…","tables_to_verify":["VBRK","VBRP"]}}
                    hint = f"Ответ невалиден. Верни JSON по схеме. Пример:\n``````"
                    messages.append({"role":"user","content":hint})
                    db.log_message(turn_index=len(messages)-1, role="user", content=hint, meta={"reason":"bad_json_example"}, dialog_id=final_dialog_id)
                continue

            # Попытка валидации схемы с обработкой ошибок
            try:
                plan = NextStep(**job)
            except ValidationError as e:
                bad_json_streak += 1
                correction = "Ответ невалиден. Верни JSON по схеме."
                messages.append({"role":"user","content":correction})
                db.log_message(turn_index=len(messages)-1, role="user", content=correction, meta={"reason":"validation_error", "error": str(e)}, dialog_id=final_dialog_id)
                if bad_json_streak >= 3:
                    example = {"next_step":{"kind":"select_tables","thought":"Пояснение…","tables_to_verify":["VBRK","VBRP"]}}
                    hint = f"Ответ невалиден. Верни JSON по схеме. Пример:\n``````"
                    messages.append({"role":"user","content":hint})
                    db.log_message(turn_index=len(messages)-1, role="user", content=hint, meta={"reason":"validation_error_example"}, dialog_id=final_dialog_id)
                continue
            ...
            db.log_message(turn_index=len(messages)-1, role="assistant", content=normalized, meta={"normalized": True}, dialog_id=final_dialog_id)
            ...
            if isinstance(step, Step_SelectTables):
                ...
                for action in step.actions:
                    if isinstance(action, Tool_GetTableFields):
                        ...
                        tool_results.append({"tool":"get_domain_texts","domaine":action.domain_name,"result": result})
                        ...
                        tool_results.append({"tool":"runsapsql_query","name":action.name,"sql":action.query,"result": result})
                ...
                db.log_message(turn_index=len(messages)-1, role="assistant", content=final_msg, meta={"final_answer": True}, dialog_id=final_dialog_id)
            ...
            if tool_results:
                ...
                db.log_message(turn_index=len(messages)-1, role="tool", content=blob, dialog_id=final_dialog_id)
...
if __name__ == "__main__":
    query = os.getenv("QUERY", "Сколько есть авиарейсов из Нью-Иорка?")
    try:
